...
for intf, vlan in trunk.items():
        ...
        for command in trunk_template:
            if command.endswith('allowed vlan') and vlan[0] == 'only':
                vlan1 = ','.join(vlan[1:])
                print(' {} {}'.format(command, vlan1))
            elif command.endswith('allowed vlan') and vlan[0] == 'del':
                vlan1 = ','.join(vlan[1:])
                print(' {} {} {}'.format(command, 'remove', vlan1))
            elif command.endswith('allowed vlan') and vlan[0] == 'add':
                vlan1 = ','.join(vlan[1:])
                print(' {} {} {}'.format(command, 'add', vlan1))
            else:
                print(' {}'.format(command))
