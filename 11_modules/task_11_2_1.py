...
def create_network_map(filenames):
    dictionary = {}
    dictionary_out_string = []
    dictionary_string = []
    for name in filenames:
        dictionary_out_string = []
        dictionary_string = []
        name = '/opt/myallgit/online-7-eugene-zayka/exercises/11_modules/' + name
        with open(name) as f:
        ...
        for key in dictionary.keys():
            for value in dictionary.values():
                if key == value:
                    dictionary_string.append(key)

    for string in dictionary_string:
        if string in dictionary.keys() and string in dictionary.values():
            del dictionary[string]


#    pprint(dictionary_string)
#    pprint(dictionary_out_string)

#            del dictionary[string]
    return dictionary


list_of_names = ['sh_cdp_n_sw1.txt', 'sh_cdp_n_r1.txt', 'sh_cdp_n_r2.txt', 'sh_cdp_n_r3.txt']
map = create_network_map(list_of_names)
draw_topology(map)
